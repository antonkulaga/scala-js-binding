...
trait ModelEndpoint extends AjaxModelEndpoint with PickleController with Items{
  ...
  override def onCreate(createMessage: Create)(implicit request: ModelRequest): ModelResult = {
    this.extractKV(createMessage.channel) match {
      case Some((key,value))=> this.items = this.items.updated(key,value++createMessage.models)
  ...
  override def onUpdate(updateMessage: Update)(implicit request: ModelRequest): ModelResult = {
    ...
    this.extractKV(updateMessage.channel) match {
      case Some((key,value))=>
        this.items = this.items.updated(key,value++updateMessage.models)
        Future.successful(TRUE )
  ...
  override def onRead(readMessage: Read)(implicit request: ModelRequest): ModelResult = {
    this.extract(readMessage.channel) match {
      case Some(items)=>
  ...
  override def onDelete(deleteMessage: Delete)(implicit request: ModelRequest): ModelResult = {
    this.extractKV(deleteMessage.channel) match {
      case Some((key,value))=>
        this.items = this.items.updated(key,value.filterNot(i=>i.id==deleteMessage.res))
  ...
  override def onSuggest(suggestMessage: Suggest): ModelResult = {
    ...
    this.extract(suggestMessage.channel) match {
