...
import org.denigma.preview.charts._
...
class CompBioView(val elem: Element) extends BindableView with Plot {
  ...
  val scaleX: rx.Var[Scale] = Var(LinearScale("Time", 0, 100, 10, 1000))

  val scaleY: rx.Var[Scale] = Var(LinearScale("TetR", 0, 100, 10, 1000, inverted = true))
  ...
  val lineXplus100Series = Rx {
    LineSeries("2*x", scaleX().start, scaleX().end, LineStyles.default.copy(strokeColor = "red"))(x => Point(x, x + 1))
  }
  val lineX2 = Rx {
  ...
  solve.handler{
    println("solve button works")
    val ode = new CompBioODEs()
    val result = ode.solve(0,0,0,0)
    println("========================================================================")
    println(s"SOLUTION IS: [ ${result.toList.mkString(" , ")} ]")
  }
